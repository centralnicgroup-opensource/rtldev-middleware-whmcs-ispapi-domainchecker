...
CategoryManager.prototype.setCategories = function (categories, activeCategories) {
  ...
  categories.sort(function (a, b) {
    ...
    const cat = new Category(d.name, d.id, d.tlds, activeCategories.indexOf(d.id) !== -1)
...
CategoryManager.prototype.handleClicks = function () {
  $('.subCat').off('click').click(function (e) {
    ...
    if (cat.id === -1) {
      ...
      if (cat.active) {
        ...
        ds.searchStore.activeCategories = ds.searchStore.activeCategories.filter(catid => {
  ...
  $('.category-button').off('click').click(function () {
...
CategoryManager.prototype.getSelectedTLDs = function () {
  ...
  this.categories.filter((category) => {
    ...
    tlds = tlds.concat(category.tlds.filter((item) => {
